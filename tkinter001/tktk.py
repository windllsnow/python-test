...
accountE=Entry(root)
pwdE=Entry(root,show="*")   # key 進去 ，顯示都是*
...
# 以下建立Login和Quit案鈕
loginbtn = Button(root,text="Login",command=printInfo)
loginbtn.grid(row=3,column=0,sticky=W,pady=5)   # 切齊
quitbtn = Button(root,text="Quit",command=root.destroy)
quitbtn.grid(row=3,column=1,sticky=W,pady=5)   # 切齊
...
print("結果是 : ", eval(expression))    #字串 傳回
