...
def build_dict(name,age,**players):
    ...
    for key,value in players.items():
        info[key]=value
    return info
kkk=build_dict('James','32',City='Cleveland',State='Ohio')
print(kkk)

#%%

# from ______ import ______,______
# from______import*
# import _______ as _____
...
for i in range(n):
    print("300-500:",random.BPFrandint(300,500))
for i in range(n):
    print("1000-2000:",random.randint(1000,2000))
    #%%
...
'''
xx=random.randint(min,max)
xxx=random.randint(min,max)
'''
   #%%
   
import random
fruits=["貧果","水蜜桃","紅龍果","香蕉","西瓜","草莓"]
print(random.choice(fruits))
    
#%%
import random
porker=['2','3','4','5','6','7','8','0','l','k','t','u']
for i in range (3):
...
dateObj=datetime.strptime("2020/11/11","%Y/%m/%d")
print(dateObj)
...
fields=["Amy",159.5,25.3]
print("{0:^10}{1:^7.1f}{2:^9.3f}".format(fields[0],fields[1],fields[2]))
#%%


#   xx=print(os.listdir("D:桌面"))
#%%

with open("D:桌面/456.txt") as file_obj:
    data=file_obj.read()
    print(data)
#%%
with open("D:桌面/456.txt") as file_obj:
...
qwe\n\
'
with open(k1,'w') as file_obj1:
    file_obj1.write(string+'\n')
    file_obj1.write(str2+'\n')
with open(k1,'r') as file_obj1:    
...
def division(x,y):
    try:
        ...
        errlog= open('err_trace.txt','a')
        errlog.write(traceback.format_exc())
        errlog.close()
        print("將Traceback寫入錯誤檔案err_trace.txt完成")
        print("異常發生")
...
if not type(x) is int:
    raise TypeError("Only integers are allowed1111")
  #%%  
   #資料處理 
